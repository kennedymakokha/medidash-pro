...
export default function BedsPage() {
  ...
  const handleSubmit = async (Data: BedData) => {
    await postBed(Data).unwrap();
    await refetch();
    toast({
      title: `${Data.uuid !== undefined ? "Update Lab" : "Lab Added"}`,
      description: ` ${Data.uuid ? "has been Updated" : "has been added"} successfully.`,
    });
  };
  const MockWards: WardData[] = wards !== undefined ? wards.data : [];

  const handleDelete = async () => {
    await postBed({ ...formData, isDeleted: true }).unwrap();
    await refetch();
    toast({
      title: "Bed Removed",
      description: `Bed has been removed.`,
      variant: "destructive",
    });
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Manage hospital beds and occupancy"
    >
      {/* Header Actions */}
