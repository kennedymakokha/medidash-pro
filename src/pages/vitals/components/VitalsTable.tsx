...
function getVitalSeverity(value: number, type: "temp" | "sys" | "dia" | "hr" | "spo2") {
  switch (type) {
    case "temp":
...
function calculateEWS(vital: any) {
  let score = 0;
    ...
    <DataTable
      ...
      rows={
        visits.length === 0 ? (
          ...
          normalized.map((vital: any) => {
            const tempStatus = getVitalSeverity(Number(vital?.temperature), "temp");
            const sysStatus = getVitalSeverity(vital.bloodPressureSystolic, "sys");
            const diaStatus = getVitalSeverity(vital.bloodPressureDiastolic, "dia");
            const hrStatus = getVitalSeverity(Number(vital.heartRate ?? vital.pulse), "hr");
            const spo2Status = getVitalSeverity(Number(vital.oxygenSaturation), "spo2");

            const bpStatus = sysStatus === "critical" || diaStatus === "critical" ? "critical"
            ...
            return (
              <tr key={vital._id ?? vital.id} className={cn("hover:bg-muted/50", status === "critical" && "bg-destructive/5")}>
                ...
                <td className={cn("px-4 py-3", highlightStyles[tempStatus])}>{vital.temperature}</td>
                <td className={cn("px-4 py-3", highlightStyles[bpStatus])}>{vital.bloodPressureSystolic}/{vital.bloodPressureDiastolic}</td>
                <td className={cn("px-4 py-3", highlightStyles[hrStatus])}>{vital.heartRate ?? vital.pulse}</td>
                <td className="px-4 py-3">{vital.respiratoryRate}</td>
                <td className={cn("px-4 py-3", highlightStyles[spo2Status])}>{vital.oxygenSaturation}%</td>
                <td className="px-4 py-3">
                  <Badge variant="outline" className={cn("capitalize", statusStyles[status])}>{status} ({score})</Badge>
                </td>
