...
export function calculateEWS(vital: any) {
  let score = 0;
...
export function VitalsStats({ vitals, setFilter }: Props) {
  const statuses = vitals.map((v: any) => getEWSStatus(calculateEWS(v)));
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      {stats.map((s) => (
        <Card key={s.label}>
          <CardContent
            onClick={() => setFilter(s.label === "Total Records" ? "all" : s.label.toLowerCase())}
